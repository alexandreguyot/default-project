            ...
            <table class="table table-index w-full">
                ...
                <tbody>
                    @forelse($contentCategories as $contentCategory)
                        <tr>
                            <td>
                                <input type="checkbox" value="{{ $contentCategory->id }}" wire:model.live="selected">
                            </td>
                            <td>
                                {{ $contentCategory->id }}
                            </td>
                            ...
                            <td>
                                <div class="flex justify-end">
                                    @can('content_category_show')
                                        <a class="btn btn-sm btn-info mr-2" href="{{ route('content-categories.show', $contentCategory) }}">
                                            {{ trans('global.view') }}
                                        </a>
                                    @endcan
                                    @can('content_category_edit')
                                        <a class="btn btn-sm btn-success mr-2" href="{{ route('content-categories.edit', $contentCategory) }}">
                                            {{ trans('global.edit') }}
                                        </a>
                                    @endcan
                                    @can('content_category_delete')
