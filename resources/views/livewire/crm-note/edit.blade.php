<form wire:submit="submit" class="pt-3">
    ...
    <div class="form-group {{ $errors->has('crmNote.customer_id') ? 'invalid' : '' }}">
        ...
        <x-select-list class="form-control" required id="customer" name="customer" :options="$this->listsForFields['customer']" wire:model.live="crmNote.customer_id" />
    ...
    <div class="form-group {{ $errors->has('crmNote.note') ? 'invalid' : '' }}">
        ...
        <textarea class="form-control" name="note" id="note" required wire:model="crmNote.note" rows="4"></textarea>
        <div class="validation-message">
    ...
    <div class="form-group">
        ...
        <a href="{{ route('crm-notes.index') }}" class="btn btn-secondary">
            {{ trans('global.cancel') }}
        </a>
    </div>
</form>
