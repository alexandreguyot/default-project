            ...
            <table class="table table-index w-full">
                ...
                <tbody>
                    @forelse($faqCategories as $faqCategory)
                        <tr>
                            <td>
                                <input type="checkbox" value="{{ $faqCategory->id }}" wire:model.live="selected">
                            </td>
                            <td>
                                {{ $faqCategory->id }}
                            </td>
                            ...
                            <td>
                                <div class="flex justify-end">
                                    @can('faq_category_show')
                                        <a class="btn btn-sm btn-info mr-2" href="{{ route('faq-categories.show', $faqCategory) }}">
                                            {{ trans('global.view') }}
                                        </a>
                                    @endcan
                                    @can('faq_category_edit')
                                        <a class="btn btn-sm btn-success mr-2" href="{{ route('faq-categories.edit', $faqCategory) }}">
                                            {{ trans('global.edit') }}
                                        </a>
                                    @endcan
                                    @can('faq_category_delete')
