            ...
            <table class="table table-index w-full">
                ...
                <tbody>
                    @forelse($taskTags as $taskTag)
                        <tr>
                            <td>
                                <input type="checkbox" value="{{ $taskTag->id }}" wire:model.live="selected">
                            </td>
                            <td>
                                {{ $taskTag->id }}
                            </td>
                            ...
                            <td>
                                <div class="flex justify-end">
                                    @can('task_tag_show')
                                        <a class="btn btn-sm btn-info mr-2" href="{{ route('task-tags.show', $taskTag) }}">
                                            {{ trans('global.view') }}
                                        </a>
                                    @endcan
                                    @can('task_tag_edit')
                                        <a class="btn btn-sm btn-success mr-2" href="{{ route('task-tags.edit', $taskTag) }}">
                                            {{ trans('global.edit') }}
                                        </a>
                                    @endcan
                                    @can('task_tag_delete')
